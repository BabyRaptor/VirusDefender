var ENEMY_3_SIZE = 100;
var ENEMY_3_HP = 75;
var ENEMY_3_SCORE = 50;
var ENEMY_3_DAMAGE = 50;
var ENEMY_3_SPAWN_RATE = 4000;

function Enemy3(layer) {
	this.x = 0;
	this.y = 0;
	this.a = 0;
	this.w = ENEMY_3_SIZE;
	...
	var coreSprite = new cc.Sprite("res/GSGamePlay/Enemy3.png");
	...
	this.Spawn = function () {
		...
		if (side == 0) {
			...
			this.x = Math.random() * CANVAS_W;
			this.y = CANVAS_H + ENEMY_3_SIZE;
		}
		else if (side == 2) {
			this.y = Math.random() * CANVAS_H;
			this.x = CANVAS_W + ENEMY_3_SIZE;
		}
		else if (side == 3) {
			this.y = Math.random() * CANVAS_H;
			this.x = -ENEMY_3_SIZE;
		...
		myAudio.PlaySound("res/GSGamePlay/ExplosionT.mp3");
	}
	this.Update = function (deltaTime) {
		if (this.live == true) {
			...
			if (this.x < -ENEMY_3_SIZE * 1.5 || this.x > CANVAS_W + ENEMY_3_SIZE * 1.5
			||  this.y < -ENEMY_3_SIZE * 1.5 || this.y > CANVAS_H + ENEMY_3_SIZE * 1.5) {
				this.Spawn();
			}
		}
	}
	this.Explode = function(earnScore) {
		if (this.live == true) {
			...
			var bulletParticle = cc.ParticleSystem.create("res/GSGamePlay/Enemy3Particle.plist");
			...
			if (earnScore)  layer.addScore(ENEMY_3_SCORE);
			else 			layer.loseScore(ENEMY_3_DAMAGE);
	...
	this.CheckCollisionWithPlayer = function(player, shield) {
		if (this.live) {
			...
			if (Math.abs (this.x - player.x) < (ENEMY_3_SIZE + checkAOE) * 0.5
			&&  Math.abs (this.y - player.y) < (ENEMY_3_SIZE + checkAOE) * 0.5) {
				this.Explode(false);
			}
		}
	}
